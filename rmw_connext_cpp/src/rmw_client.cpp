...
rmw_client_t *rmw_create_client(
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(node, node->implementation_identifier,
                                   rti_connext_identifier, return NULL);
  ...
  if (!_process_service_name(service_name,
                             qos_profile->avoid_ros_namespace_conventions,
                             &request_topic_str, &response_topic_str)) {
    goto fail;
  }

  requester = callbacks->create_requester(
      participant, request_topic_str, response_topic_str, &datareader_qos,
      &datawriter_qos, reinterpret_cast<void **>(&response_datareader),
      reinterpret_cast<void **>(&request_datawriter), &rmw_allocate);
  ...
  // Use a placement new to construct the ConnextStaticClientInfo in the
  // preallocated buffer. cppcheck-suppress syntaxError
  RMW_TRY_PLACEMENT_NEW(client_info, buf, goto fail, ConnextStaticClientInfo, )
  buf = nullptr; // Only free the client_info pointer; don't need the buf
                 // pointer anymore.
  ...
  memcpy(const_cast<char *>(client->service_name), service_name,
  ...
  node_info->subscriber_listener->add_information(
      ...
      response_datareader->get_instance_handle(), mangled_name,
  ...
  node_info->publisher_listener->add_information(
      node_info->participant->get_instance_handle(),
      request_datawriter->get_instance_handle(), mangled_name,
      request_datawriter->get_topic()->get_type_name(), actual_qos_profile,
...
fail:
  ...
  if (response_datareader && dds_subscriber) {
    if (dds_subscriber->delete_datareader(response_datareader) !=
        DDS::RETCODE_OK) {
      std::stringstream ss;
  ...
  // TODO(wjwwood): deallocate requester (currently allocated with new
  // elsewhere)
  if (client_info) {
...
rmw_ret_t rmw_destroy_client(rmw_node_t *node, rmw_client_t *client) {
  ...
  if (client_info) {
    ...
    node_info->subscriber_listener->remove_information(
        client_info->response_datareader_->get_instance_handle(),
        EntityType::Subscriber);
    DDS::DataWriter *request_datawriter = static_cast<DDS::DataWriter *>(
        client_info->callbacks_->get_request_datawriter(
            client_info->requester_));
    node_info->subscriber_listener->trigger_graph_guard_condition();

    node_info->publisher_listener->remove_information(
        request_datawriter->get_instance_handle(), EntityType::Publisher);
    node_info->publisher_listener->trigger_graph_guard_condition();

    if (response_datareader) {
      auto read_condition = client_info->read_condition_;
      if (read_condition) {
        if (response_datareader->delete_readcondition(read_condition) !=
            DDS::RETCODE_OK) {
      ...
      RMW_SET_ERROR_MSG(
          "cannot delete readcondition because the datareader is null");
      result = RMW_RET_ERROR;
    }
    ...
    RMW_TRY_DESTRUCTOR(client_info->~ConnextStaticClientInfo(),
