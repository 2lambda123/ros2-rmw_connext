...
rmw_publisher_t *rmw_create_publisher(
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(node handle, node->implementation_identifier,
                                   rti_connext_identifier, return nullptr);
  ...
  auto participant =
      static_cast<DDS::DomainParticipant *>(node_info->participant);
  const message_type_support_callbacks_t *callbacks =
      static_cast<const message_type_support_callbacks_t *>(type_support->data);

  std::string type_name = _create_type_name(callbacks);
  // Past this point, a failure results in unrolling code in the goto fail
  // block.
  ...
  // advertise the topic however with a type of the message, e.g.
  ...
  if (!_process_topic_name(topic_name,
  ...
  // Use a placement new to construct the PublisherListener in the preallocated
  // buffer. cppcheck-suppress syntaxError
  RMW_TRY_PLACEMENT_NEW(publisher_listener, listener_buf, goto fail,
  ...
  topic = rmw_connext_shared_cpp::create_topic(node, topic_name, topic_str,
                                               type_name.c_str());
  if (!topic) {
    // error already set
    goto fail;
  ...
  topic_writer = dds_publisher->create_datawriter(topic, datawriter_qos, NULL,
  ...
  // Use a placement new to construct the ConnextStaticPublisherInfo in the
  // preallocated buffer.
  RMW_TRY_PLACEMENT_NEW(publisher_info, info_buf, goto fail,
                        ConnextStaticPublisherInfo, )
  info_buf = nullptr; // Only free the publisher_info pointer; don't need the
                      // buf pointer anymore.
  ...
  publisher_info->publisher_gid.implementation_identifier =
      rti_connext_identifier;
  publisher_info->listener_ = publisher_listener;
  publisher_listener = nullptr;
  static_assert(sizeof(ConnextPublisherGID) <= RMW_GID_STORAGE_SIZE,
                "RMW_GID_STORAGE_SIZE insufficient to store the "
                "rmw_connext_cpp GID implemenation.");
  // Zero the data memory.
  memset(publisher_info->publisher_gid.data, 0, RMW_GID_STORAGE_SIZE);
  {
    auto publisher_gid = reinterpret_cast<ConnextPublisherGID *>(
  ...
  memcpy(const_cast<char *>(publisher->topic_name), topic_name,
         strlen(topic_name) + 1);
  ...
  node_info->publisher_listener->add_information(
      ...
      dds_publisher->get_instance_handle(), mangled_name, type_name,
      actual_qos_profile, EntityType::Publisher);
...
fail:
  ...
  if (dds_publisher) {
    if (topic_writer) {
      if (dds_publisher->delete_datawriter(topic_writer) != DDS::RETCODE_OK) {
        std::stringstream ss;
        ss << "leaking datawriter while handling failure at " << __FILE__ << ":"
           << __LINE__ << '\n';
    ...
    if (participant->delete_publisher(dds_publisher) != DDS::RETCODE_OK) {
      std::stringstream ss;
      ss << "leaking publisher while handling failure at " << __FILE__ << ":"
         << __LINE__ << '\n';
      (std::cerr << ss.str()).flush();
    }
  }
  if (topic) {
    if (participant->delete_topic(topic) != DDS::RETCODE_OK) {
      std::stringstream ss;
      ss << "leaking topic while handling failure at " << __FILE__ << ":"
         << __LINE__ << '\n';
      (std::cerr << ss.str()).flush();
    }
  }
  if (publisher_listener) {
    RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(
        publisher_listener->~ConnextPublisherListener(),
        ConnextPublisherListener)
  ...
  if (publisher_info) {
    if (publisher_info->listener_) {
      RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(
          publisher_info->listener_->~ConnextPublisherListener(),
          ConnextPublisherListener)
...
rmw_ret_t rmw_publisher_get_actual_qos(const rmw_publisher_t *publisher,
  ...
  RMW_CHECK_ARGUMENT_FOR_NULL(publisher, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(
      publisher handle, publisher->implementation_identifier,
      rti_connext_identifier, return RMW_RET_INCORRECT_RMW_IMPLEMENTATION);
...
rmw_borrow_loaned_message(const rmw_publisher_t *publisher,
  ...
  RMW_SET_ERROR_MSG(
      "rmw_borrow_loaned_message not implemented for rmw_connext_cpp");
  return RMW_RET_UNSUPPORTED;
}
...
rmw_ret_t rmw_destroy_publisher(rmw_node_t *node, rmw_publisher_t *publisher) {
  ...
  auto participant =
      static_cast<DDS::DomainParticipant *>(node_info->participant);
  // TODO(wjwwood): need to figure out when to unregister types with the
  // participant.
  ...
  node_info->publisher_listener->remove_information(
      publisher_info->dds_publisher_->get_instance_handle(),
      EntityType::Publisher);
  node_info->publisher_listener->trigger_graph_guard_condition();
  DDS::Publisher *dds_publisher = publisher_info->dds_publisher_;

  if (dds_publisher->delete_datawriter(publisher_info->topic_writer_) !=
      DDS::RETCODE_OK) {
  ...
  if (RMW_RET_OK == ret) {
    RMW_TRY_DESTRUCTOR(pub_listener->~ConnextPublisherListener(),
                       ConnextPublisherListener, ret = RMW_RET_ERROR);
  } else {
    RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(
        pub_listener->~ConnextPublisherListener(), ConnextPublisherListener);
  }
  rmw_free(pub_listener);

  if (RMW_RET_OK == ret) {
    RMW_TRY_DESTRUCTOR(publisher_info->~ConnextStaticPublisherInfo(),
