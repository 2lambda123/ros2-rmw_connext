...
rmw_service_t *rmw_create_service(
  ...
  replier = callbacks->create_replier(
      ...
      &datawriter_qos, reinterpret_cast<void **>(&request_datareader),
      reinterpret_cast<void **>(&response_datawriter), &rmw_allocate);
  ...
  // Use a placement new to construct the ConnextStaticServiceInfo in the
  // preallocated buffer. cppcheck-suppress syntaxError
  RMW_TRY_PLACEMENT_NEW(service_info, buf, goto fail,
                        ConnextStaticServiceInfo, )
  buf = nullptr; // Only free the service_info pointer; don't need the buf
                 // pointer anymore.
  ...
  memcpy(const_cast<char *>(service->service_name), service_name,
  ...
  node_info->subscriber_listener->add_information(
      ...
      request_datareader->get_instance_handle(), mangled_name,
  ...
  node_info->publisher_listener->add_information(
      node_info->participant->get_instance_handle(),
      response_datawriter->get_instance_handle(), mangled_name,
      response_datawriter->get_topic()->get_type_name(), actual_qos_profile,
...
fail:
  ...
  if (request_datareader) {
    if (read_condition) {
      if (request_datareader->delete_readcondition(read_condition) !=
          DDS::RETCODE_OK) {
        std::stringstream ss;
    ...
    if (dds_subscriber) {
      if (dds_subscriber->delete_datareader(request_datareader) !=
          DDS::RETCODE_OK) {
        std::stringstream ss;
      ...
      RMW_SET_ERROR_MSG(
          "cannot delete datareader because the subscriber is null");
    }
  }
  if (service_info) {
...
rmw_ret_t rmw_destroy_service(rmw_node_t *node, rmw_service_t *service) {
  ...
  if (service_info) {
    ...
    RMW_TRY_DESTRUCTOR(service_info->~ConnextStaticServiceInfo(),
