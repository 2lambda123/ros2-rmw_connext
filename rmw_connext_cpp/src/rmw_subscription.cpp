...
rmw_subscription_t *rmw_create_subscription(
  ...
  // subscribe the topic however with a type of the message, e.g.
  ...
  // Use a placement new to construct the ConnextSubscriberListener in the
  // preallocated buffer. cppcheck-suppress syntaxError
  RMW_TRY_PLACEMENT_NEW(subscriber_listener, listener_buf, goto fail,
  ...
  topic_reader = dds_subscriber->create_datareader(topic, datareader_qos, NULL,
  ...
  // Use a placement new to construct the ConnextStaticSubscriberInfo in the
  // preallocated buffer.
  RMW_TRY_PLACEMENT_NEW(subscriber_info, info_buf, goto fail,
                        ConnextStaticSubscriberInfo, )
  info_buf = nullptr; // Only free the subscriber_info pointer; don't need the
                      // buf pointer anymore.
  ...
  memcpy(const_cast<char *>(subscription->topic_name), topic_name,
         strlen(topic_name) + 1);
  ...
  node_info->subscriber_listener->add_information(
      ...
      dds_subscriber->get_instance_handle(), mangled_name, type_name,
      actual_qos_profile, EntityType::Subscriber);
...
fail:
  ...
  if (dds_subscriber) {
    if (topic_reader) {
      if (read_condition) {
        if (topic_reader->delete_readcondition(read_condition) !=
            DDS::RETCODE_OK) {
          std::stringstream ss;
          ss << "leaking readcondition while handling failure at " << __FILE__
             << ":" << __LINE__ << '\n';
      ...
      if (dds_subscriber->delete_datareader(topic_reader) != DDS::RETCODE_OK) {
        std::stringstream ss;
        ss << "leaking datareader while handling failure at " << __FILE__ << ":"
           << __LINE__ << '\n';
        (std::cerr << ss.str()).flush();
      }
    }
    if (participant->delete_subscriber(dds_subscriber) != DDS::RETCODE_OK) {
      std::stringstream ss;
      std::cerr << "leaking subscriber while handling failure at " << __FILE__
                << ":" << __LINE__ << '\n';
      (std::cerr << ss.str()).flush();
    }
  }
  if (topic) {
    if (participant->delete_topic(topic) != DDS::RETCODE_OK) {
      std::stringstream ss;
      std::cerr << "leaking topic while handling failure at " << __FILE__ << ":"
                << __LINE__ << '\n';
      (std::cerr << ss.str()).flush();
    }
  }
  ...
  if (subscriber_info) {
    if (subscriber_info->listener_) {
      RMW_TRY_DESTRUCTOR_FROM_WITHIN_FAILURE(
          subscriber_info->listener_->~ConnextSubscriberListener(),
          ConnextSubscriberListener)
...
rmw_ret_t rmw_destroy_subscription(rmw_node_t *node,
  ...
  if (RMW_RET_OK == ret) {
    RMW_TRY_DESTRUCTOR(subscriber_listener->~ConnextSubscriberListener(),
  ...
  if (RMW_RET_OK == ret) {
    RMW_TRY_DESTRUCTOR(subscriber_info->~ConnextStaticSubscriberInfo(),
