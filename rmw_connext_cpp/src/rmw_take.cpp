...
static bool take(DDS::DataReader *dds_data_reader,
                 bool ignore_local_publications,
                 rcutils_uint8_array_t *cdr_stream, bool *taken,
  ...
  DDS::ReturnCode_t status =
      data_reader->take(dds_messages, sample_infos, 1, DDS::ANY_SAMPLE_STATE,
  ...
  if (!sample_info.valid_data) {
    ...
    // compare the lower 12 octets of the guids from the sender and this
    // receiver if they are equal the sample has been sent from this process and
    // should be ignored
    DDS::GUID_t sender_guid = sample_info.original_publication_virtual_guid;
    DDS::InstanceHandle_t receiver_instance_handle =
        dds_data_reader->get_instance_handle();
    ignore_sample = true;
  ...
  if (!ignore_sample) {
    ...
    cdr_stream->buffer = reinterpret_cast<uint8_t *>(
        malloc(cdr_stream->buffer_length * sizeof(uint8_t)));

    if (cdr_stream->buffer_length >
        (std::numeric_limits<unsigned int>::max)()) {
      RMW_SET_ERROR_MSG("cdr_stream->buffer_length unexpectedly larger than "
                        "max unsiged int value");
      data_reader->return_loan(dds_messages, sample_infos);
      *taken = false;
      return false;
    }
    memcpy(cdr_stream->buffer, &dds_messages[0].serialized_data[0],
           cdr_stream->buffer_length);
...
extern "C" {
rmw_ret_t _take(const rmw_subscription_t *subscription, void *ros_message,
                bool *taken, DDS::InstanceHandle_t *sending_publication_handle,
                rmw_subscription_allocation_t *allocation) {
  if (!subscription) {
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(subscription handle,
                                   ...
                                   rti_connext_identifier, return RMW_RET_ERROR)
  ...
  if (!take(topic_reader, subscription->options.ignore_local_publications,
            &cdr_stream, taken, sending_publication_handle, allocation)) {
...
rmw_ret_t rmw_take_with_info(const rmw_subscription_t *subscription,
  ...
  auto ret = _take(subscription, ros_message, taken,
...
rmw_ret_t
_take_serialized_message(const rmw_subscription_t *subscription,
...
rmw_take_serialized_message(const rmw_subscription_t *subscription,
                            ...
                            bool *taken,
                            rmw_subscription_allocation_t *allocation) {
  return _take_serialized_message(subscription, serialized_message, taken,
                                  nullptr, allocation);
}
...
rmw_ret_t rmw_take_serialized_message_with_info(
  ...
  DDS::InstanceHandle_t sending_publication_handle;
  auto ret = _take_serialized_message(subscription, serialized_message, taken,
                                      &sending_publication_handle, allocation);
  if (ret != RMW_RET_OK) {
    // Error string is already set.
    return RMW_RET_ERROR;
...
rmw_ret_t rmw_take_loaned_message(const rmw_subscription_t *subscription,
                                  const void **loaned_message, bool *taken,
                                  rmw_subscription_allocation_t *allocation) {
  (void)subscription;
  (void)loaned_message;
  (void)taken;
  ...
  RMW_SET_ERROR_MSG(
      "rmw_take_loaned_message not implemented for rmw_connext_cpp");
...
rmw_take_loaned_message_with_info(const rmw_subscription_t *subscription,
  ...
  RMW_SET_ERROR_MSG(
      "rmw_take_loaned_message_with_info not implemented for rmw_connext_cpp");
  return RMW_RET_UNSUPPORTED;
}

rmw_ret_t rmw_return_loaned_message_from_subscription(
    const rmw_subscription_t *subscription, const void *loaned_message) {
  (void)subscription;
  (void)loaned_message;
