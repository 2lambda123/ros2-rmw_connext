...
bool set_entity_qos_from_profile_generic(const rmw_qos_profile_t &qos_profile,
  ...
  if (entity_qos.history.kind == DDS::KEEP_LAST_HISTORY_QOS &&
    ...
    if (qos_profile.depth >
        static_cast<size_t>((std::numeric_limits<DDS::Long>::max)())) {
      RMW_SET_ERROR_MSG("failed to set history depth since the requested queue "
                        "size exceeds the DDS type");
      return false;
    }
    entity_qos.history.depth = static_cast<DDS::Long>(qos_profile.depth);
  }
...
bool set_entity_qos_from_profile(const rmw_qos_profile_t &qos_profile,
  ...
  // Set any QoS settings that are specific to DataReader, then call the shared
  // version
...
bool set_entity_qos_from_profile(const rmw_qos_profile_t &qos_profile,
  ...
  // Set any QoS settings that are specific to DataWriter, then call the shared
  // version
...
bool get_datareader_qos(DDS::DomainParticipant *participant,
  ...
  DDS::ReturnCode_t status =
      participant->get_default_datareader_qos(datareader_qos);
  ...
  status = DDS::PropertyQosPolicyHelper::add_property(
      datareader_qos.property,
      "reader_resource_limits.dynamically_allocate_fragmented_samples", "1",
      DDS::BOOLEAN_FALSE);
  if (status != DDS::RETCODE_OK) {
    RMW_SET_ERROR_MSG("failed to add qos property");
    return false;
...
void dds_qos_lifespan_to_rmw_qos_lifespan(const AttributeT &dds_qos,
...
template <>
void dds_qos_lifespan_to_rmw_qos_lifespan<DDS::DataReaderQos>(
    const DDS::DataReaderQos & /*dds_qos*/, rmw_qos_profile_t * /*qos*/) {
  // lifespan does does not exist in DataReader, so no-op here
}

template <>
void dds_qos_lifespan_to_rmw_qos_lifespan<DDS::SubscriptionBuiltinTopicData>(
...
template <typename AttributeT>
void dds_qos_to_rmw_qos(const AttributeT &dds_qos, rmw_qos_profile_t *qos) {
...
template void
dds_qos_to_rmw_qos<DDS::DataWriterQos>(const DDS::DataWriterQos &dds_qos,
                                       rmw_qos_profile_t *qos);

template void
dds_qos_to_rmw_qos<DDS::DataReaderQos>(const DDS::DataReaderQos &dds_qos,
                                       rmw_qos_profile_t *qos);

template <typename AttributeT>
void dds_remote_qos_to_rmw_qos(const AttributeT &dds_qos,
                               rmw_qos_profile_t *qos) {
  qos->history = RMW_QOS_POLICY_HISTORY_UNKNOWN;
...
template void dds_remote_qos_to_rmw_qos<DDS::PublicationBuiltinTopicData>(
    const DDS::PublicationBuiltinTopicData &dds_qos, rmw_qos_profile_t *qos);

template void dds_remote_qos_to_rmw_qos<DDS::SubscriptionBuiltinTopicData>(
    const DDS::SubscriptionBuiltinTopicData &dds_qos, rmw_qos_profile_t *qos);
